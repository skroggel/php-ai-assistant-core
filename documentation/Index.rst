.. _ai-core--developer-documentation:
...
1. `Architecture <concepts/architecture.rst>`__ - components and
   responsibilities.
2. `Assistant request lifecycle <workflows/assistant-request.rst>`__ -
   what happens during one chat turn.
3. `Pipeline runtime <concepts/pipeline.rst>`__ - step resolution,
   skipping, failures and streaming.
4. `Context and state <concepts/context.rst>`__ - how processors exchange
   data.
5. `Prompt construction <concepts/prompting.rst>`__ - system prompt,
   history and dynamic context.
6. `Retrieval <concepts/retrieval.rst>`__ - embeddings, vector search and
   retrieval groups.
7. `Indexing <workflows/indexing.rst>`__ - chunking, embeddings,
   generations and vector writes.
8. `Extension points <extending/overview.rst>`__ - where custom
...
- **Provider code is behind connector contracts.** Chat/embedding
  providers implement ``AiConnectorInterface``; vector databases
  implement ``VectorStoreConnectorInterface``.
...
+============================+=========================================+
| ``Assistant/Application``  | Assistant entry points and              |
|                            | orchestration                           |
+----------------------------+-----------------------------------------+
| ``Assistant/Context``      | Runtime state shared between steps      |
+----------------------------+-----------------------------------------+
| ``Assistant/Pipeline``     | Step execution, validation and          |
|                            | processor lookup                        |
+----------------------------+-----------------------------------------+
| ``Assistant/Prompt``       | Prompt assembly and extensible prompt   |
|                            | context                                 |
+----------------------------+-----------------------------------------+
| ``Assistant/Memory``       | Conversation and retrieval-state        |
|                            | persistence                             |
+----------------------------+-----------------------------------------+
...
+----------------------------+-----------------------------------------+
| ``Connection/Resilience``  | Retry and provider error handling       |
|                            | infrastructure                          |
+----------------------------+-----------------------------------------+
| ``Indexing``               | Source normalization, chunking,         |
...
not just class signatures. The `API reference <reference/api.rst>`__
