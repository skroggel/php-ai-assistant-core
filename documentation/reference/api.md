...
This is a navigation-oriented reference. For method-level parameter
details, the PHPDoc in the source remains the canonical low-level
reference.
...
Provides assistant identity/rules, AI/vector connections and pipeline
steps.

### `PipelineStepConfigurationInterface`

Provides processor type/identifier/stage, prompt flags, history policy,
model options, retrieval options, context limits, source metadata fields
...
Shared state with assistant, request, history, retrieval, answer,
current query and processing trace.

### `RetrievalResult`

Owns retrieval groups/documents, raw-result counts and optional
optimized answer context.
...
Concrete implementations in the source include `OpenAiConnector` and
`GeminiConnector`.
...
A concrete Qdrant implementation is provided by
...
Owns conversation history and last-retrieval persistence for a chat
identifier.

The source includes session-backed memory via `SessionMemory` and
`PhpSessionStore`.
...
Provider-independent chunk/embed/upsert service for one
`IndexableDocument`.
