...
``VectorDocumentIndexer`` converts one normalized ``IndexableDocument``
into vector documents and updates a vector-store collection safely.
...
``SourceIdentityGenerator`` creates a stable source hash and
deterministic vector-document IDs based on source identity plus chunk
index.
