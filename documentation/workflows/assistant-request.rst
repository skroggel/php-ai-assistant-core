...
``createStreamProducer()`` separates preparation from emission. It
starts memory, resolves steps, opens chat logging, loads history and
creates the context immediately; it returns a closure that later
performs ``Pipeline::runStream()`` and then persists the messages.
...
with ``DirectInteraction`` settings. Conversation persistence only
occurs when ``DirectInteraction::$remember`` is true.
