...
``ProcessorRegistry::get()`` first finds the processor with the
requested identifier and then verifies that ``supports($type)`` returns
``true``.
...
``retrieval``, ``post_retrieval``, ``pre_answer``, ``post_answer``).
Execution order, however, is the order of the provided step iterable;
stage is metadata/semantics rather than an internal scheduler.
...
Sources are deduplicated primarily by ``source_identifier``, then
``url``, then a hash of the source payload.
