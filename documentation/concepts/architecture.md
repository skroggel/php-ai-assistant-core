...
2.  an **indexing path**, which turns source content into vectors used
    by retrieval.
...
`Assistant\Application\Orchestrator` owns the lifecycle of one assistant
interaction. It is responsible for:
...
`Assistant\Pipeline\Pipeline` executes configured steps in the order it
receives them. For every step it:
...
`Assistant\Context\Context` is the shared mutable state of one pipeline
run. It prevents tight coupling between processors. A query optimizer
can mutate `currentQuery`; a retriever stores retrieval groups; an
answer generator writes the answer candidate; a quality gate writes the
final answer.
...
service. Higher-level source-specific `IndexerInterface` implementations
can use adapters and this service to build concrete indexing workflows.
...
`VectorStoreConnectorInterface`, `MemoryInterface`, `AdapterInterface`,
`IndexerInterface`, `ContextBuilderInterface`) rather than on concrete
