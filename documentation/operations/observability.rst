...
``ProcessingTrace`` lives in the runtime ``Context`` and is written
directly by processors. Built-in processors use event names such as:
...
When ``canProcess()`` returns false, ``Pipeline`` records
``step.skipped`` with reason ``Required context slot missing.`` and
proceeds.
...
A thrown processor exception is logged as ``step.failed``. It is
rethrown when:
...
``ExceptionClassifier`` and ``RetryExhaustedException``. Provider
connectors should use this infrastructure consistently so transient
failures can be retried without duplicating retry logic in pipeline
processors.
