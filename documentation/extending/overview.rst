...
+----------------------------------+-------------------------------------------+
| Add source-file normalization    | ``AdapterInterface`` /                    |
|                                  | ``MultiDocumentAdapterInterface``         |
+----------------------------------+-------------------------------------------+
| Add a source-specific indexing   | ``IndexerInterface``                      |
| workflow                         |                                           |
+----------------------------------+-------------------------------------------+
...
- Keep ``getIdentifier()`` stable; configuration persists this
  identifier.
- ``supports()`` is a configuration/registration guard, not a runtime
  precondition.
- Put runtime prerequisites in ``canProcess()`` so missing state causes
  a clean skip.
...
The registry combines all supporting builders and sorts
``PromptSection``\ s by priority.
...
Use ``AdapterInterface`` when one source file maps to one normalized
text document. Use ``MultiDocumentAdapterInterface`` when one source may
